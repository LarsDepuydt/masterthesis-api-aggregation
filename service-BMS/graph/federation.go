...
func (ec *executionContext) resolveEntity(
	...
	case "Sensor":
		resolverName, err := entityResolverNameForSensor(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Sensor": %w`, err)
		}
		switch resolverName {

		case "findSensorByExternalID":
			id0, err := ec.unmarshalNString2string(ctx, rep["externalID"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findSensorByExternalID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindSensorByExternalID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Sensor": %w`, err)
			}
...
func entityResolverNameForSensor(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"externalID\" for Sensor", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Sensor", ErrTypeNotFound))
			break
		}
		return "findSensorByExternalID", nil
	}
	return "", fmt.Errorf("%w for Sensor due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
